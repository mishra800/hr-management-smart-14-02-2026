    ...
    <div className="bg-white rounded-lg shadow p-6">
      ...
      {currentImage && !capturedImage && (
        <div className="mb-4 text-center">
          <img 
            src={currentImage.startsWith('data:') || currentImage.startsWith('http') 
              ? currentImage 
              : currentImage.startsWith('/') 
                ? `${API_BASE_URL}${currentImage}`
                : `${API_BASE_URL}/uploads/profile_images/${currentImage.split('/').pop()}`
            } 
            alt="Profile" 
            className="w-32 h-32 rounded-full mx-auto border-4 border-gray-200 object-cover"
            onError={(e) => {
              console.error('Failed to load image:', currentImage);
              // Set a default avatar instead of hiding
